...
if strcmp(params{2},'mean_squared')
    error = @(s2,y) 0.5*norm(y - sigmoid(s2))^2;                            % Error scalar
    ...
    error = @(s2,y) -(y.* log(sigmoid(s2)) + (1-y).*log(1-sigmoid(s2)))*ones(n_out,1);
